...
void findMSF_prim(graph &g, graph &sf, int start)
...
vector<kru> k;
void visitNodes_kruskal(int start, graph &g)
	// Visit all nodes reachable from the start node
{
	bool found = false;
	...
	while (!found && v < g.numNodes())
	{
		...
		if (g.isEdge(start, v) && !g.isVisited(v))
		{
			obj.source=start;
			obj.destination=v;
			obj.weight=g.getEdgeWeight(start,v);
			k.push_back(obj);
			//visitNodes(v, g);
		}
		v++;
...
void kruskal(graph &g, graph &msf)
{
	for (int i=0; i<g.numNodes(); i++)
		visitNodes_kruskal(i,g);

	selectsort(k);

	for(int i=0; i< k.size() - 1; i++)
	{
		msf.addEdge(k[i].source,k[i].destination, k[i].weight);
		msf.addEdge(k[i].destination,k[i].source, k[i].weight);

		// delete edges if we make a cycle
		if (isCyclic(msf))
		{
		msf.removeEdge(k[i].source, k[i].destination);
		msf.removeEdge(k[i].destination, k[i].source);
		}
...
void selectsort(vector<kru> &k)
{
	...
	for (int i = 0; i < k.size() - 1; i++)
	{
		...
		for ( int j = i + 1; j < k.size() - 1; j++)
		{
			if (k[j].weight < k[min].weight)
			{
				min = j;
			} // if
		} // for
